...
function toCourseCard(c: RawCourse): CourseCardData {
  return {
    slug: c.slug,
    title: c.title,
    level: (c.level || "Intermediate").trim(),
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      type: "website",
      url,
      title: `${topic.label} Courses & Tracks`,
      description: topicMetaDescription(topic, count),
    },
    twitter: {
...
export default async function TopicPage({
  ...
  const featuredCourse = [...matchedCourses].sort(
    (a, b) => (b.totalStudents || 0) - (a.totalStudents || 0),
  )[0];
  const featuredTrack = tracks[0];

  const totalHours = matchedCourses.reduce(
    (s, c) => s + (c.totalDuration || 0),
    0,
  );
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
            <div className="max-w-2xl">
              ...
              <p className="text-lg text-slate-400 mb-10 leading-relaxed max-w-lg">
                {topicIntro(topic, matchedCourses.length)}
              </p>
              <div className="mb-10">
                <Button
              ...
              <div className="flex flex-wrap items-center gap-3">
                <span className="bg-slate-800/60 text-slate-300 text-xs font-medium px-3 py-1.5 rounded-md border border-slate-700/50 flex items-center gap-1.5">
                  <BookOpen className="w-3.5 h-3.5" /> {matchedCourses.length}{" "}
                  Courses
                </span>
                {totalHours > 0 && (
                  <span className="bg-slate-800/60 text-slate-300 text-xs font-medium px-3 py-1.5 rounded-md border border-slate-700/50 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5" /> {totalHours} hr
                  </span>
                )}
                <span className="bg-slate-800/60 text-slate-300 text-xs font-medium px-3 py-1.5 rounded-md border border-slate-700/50 flex items-center gap-1.5">
            ...
            <div className="w-full max-w-[520px] mx-auto lg:ml-auto">
              <div className="bg-white rounded-2xl p-8 text-slate-900 shadow-2xl">
                ...
                <p className="text-sm text-slate-500 text-center mb-6">
                  Start {topic.label} today — no credit card required.
                </p>
                <Button
                  className="w-full bg-[#13AECE] hover:bg-[#0f8b9e] text-white border-0 h-12 rounded-md font-semibold mb-3"
                  asChild
                >
                  <Link
                    href={topicAppUrl}
      ...
      <section className="bg-white text-slate-900 py-16">
        <div className="container mx-auto px-6 max-w-6xl">
          <h2 className="text-[2rem] font-extrabold text-[#0B152A] mb-2">
            Recommended for {topic.label} beginners
          </h2>
          <p className="text-slate-500 mb-8">
            Curated by working backend engineers to get you building fast.
          </p>
          {featuredCourse || featuredTrack ? (
            <div className="grid md:grid-cols-2 gap-6">
              {featuredCourse && (
                <CourseCard course={toCourseCard(featuredCourse)} />
              )}
      ...
      <section className="bg-white text-slate-900 py-16">
        <div className="container mx-auto px-6 max-w-6xl">
          <h2 className="text-[2rem] font-extrabold text-[#0B152A] mb-8">
            Related resources on {topic.label}
          </h2>
                ...
                <div className="flex flex-wrap gap-3 mb-8">
                  {related.map((r) => (
                    <Link
                      key={r.slug}
                      href={`/topic/${r.slug}`}
                      className="px-4 py-2 rounded-full bg-[#f8fafc] border border-slate-200 text-sm font-medium text-slate-700 hover:border-[#13AECE] hover:text-[#13AECE] transition-colors"
                    >
                      {r.label}
                    </Link>
                  ))}
      ...
      <section className="bg-[#f8fafc] text-slate-900 py-16">
        <div className="container mx-auto px-6 max-w-6xl">
          <h2 className="text-[2rem] font-extrabold text-[#0B152A] mb-2">
            Ready to apply your {topic.label} skills?
          </h2>
          <p className="text-slate-500 mb-8">
          ...
          {projectMatches.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
              {projectMatches.slice(0, 6).map((p) => (
                <Link
                  key={p.id}
                  href={`https://app.masteringbackend.com/projects/${p.slug}?ref=${encodeURIComponent(`/topic/${topic.slug}`)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col hover:shadow-md transition-shadow group"
                >
                  <div className="p-6 flex-1 flex flex-col">
                    <span className="text-[10px] font-bold text-slate-400 tracking-widest uppercase mb-2">
                      Project
                    </span>
                    <h4 className="text-lg font-bold text-[#0B152A] mb-2 leading-tight group-hover:text-[#13AECE] transition-colors line-clamp-2">
                      {p.title}
                    </h4>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-8 text-center">
              <p className="text-slate-600 mb-4">
                {emptyCopy("projects", topic)}
              </p>
              <Button
                className="bg-[#13AECE] hover:bg-[#0f8b9e] text-white border-0 rounded-md"
                asChild
              >
                <Link
