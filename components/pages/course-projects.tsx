...
import { ArrowLeft, FolderOpen, Clock, CheckCircle, XCircle, Star, Filter, Calendar } from "lucide-react"
...
export function CourseProjectsPage({ courseId, onNavigate }: CourseProjectsPageProps) {
  ...
  const projects = [
    {
      ...
      grade: 95,
      requirements: ["Responsive design", "Navigation menu", "About section", "Projects showcase", "Contact form"],
    },
    {
      id: "project-2",
      ...
      feedback: "",
      grade: null,
      requirements: ["Add/delete todos", "Mark as complete", "Filter todos", "Local storage", "Responsive design"],
    },
    {
      id: "project-3",
      title: "Weather Dashboard",
      description: "Build a weather app using external APIs and modern JavaScript",
      difficulty: "Advanced",
      estimatedHours: 16,
      points: 400,
      dueDate: "2024-02-25",
  ...
  const totalPoints = projects.filter((p) => p.status === "completed").reduce((acc, p) => acc + p.points, 0)
        ...
        {filteredProjects.map((project) => (
          <Card key={project.id} className="hover:shadow-lg transition-shadow">
            ...
            <CardContent className="space-y-4">
                  ...
                  {project.requirements.slice(0, 3).map((req, index) => (
                    <div key={index} className="flex items-center gap-2 text-sm">
                      <div className="w-4 h-4 rounded border border-gray-300 flex items-center justify-center">
                        {project.progress > (index + 1) * 20 && <CheckCircle className="h-3 w-3 text-green-600" />}
                      </div>
                      <span className="text-gray-600">{req}</span>
                    </div>
                  ))}
                  {project.requirements.length > 3 && (
                    <p className="text-xs text-gray-500 ml-6">+{project.requirements.length - 3} more requirements</p>
                  )}
                </div>
              </div>
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {project.grade && <p className="text-sm text-green-600">Grade: {project.grade}%</p>}
                    </div>
                    <CheckCircle className="h-5 w-5 text-green-600" />
                  </div>
                  {project.feedback && <p className="text-sm text-green-700 mt-2">{project.feedback}</p>}
                </div>
              )}
              ...
              <Button
                className="w-full"
                onClick={() => onNavigate(routes.courseProject(courseId, project.id))}
      ...
      {filteredProjects.length === 0 && (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-gray-500">No projects match your current filter.</p>
          </CardContent>
        </Card>
      )}
    </div>
