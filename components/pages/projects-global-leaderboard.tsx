        ...
        <>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {currentUser.completedProjects || 0} projects • {currentUser.totalPoints?.toLocaleString() || 0} pts
                      </p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {currentUser.averageScore || 0}% avg
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          {topUsers.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* 2nd Place */}
              {topUsers[1] && (
                <Card className={`border-2 border-gray-400 bg-gray-50 dark:bg-gray-900/20 ${isCurrentUser(topUsers[1]) ? "ring-2 ring-primary" : ""}`}>
                  ...
                  <CardContent className="text-center space-y-2">
                    <Avatar className={`h-16 w-16 mx-auto ${isCurrentUser(topUsers[1]) ? "ring-2 ring-primary" : ""}`}>
              ...
              {/* 1st Place */}
              {topUsers[0] && (
                <Card className={`border-2 border-yellow-500 bg-yellow-50 dark:bg-yellow-900/10 md:-mt-4 ${isCurrentUser(topUsers[0]) ? "ring-2 ring-primary" : ""}`}>
                  <CardHeader className="text-center pb-3">
                  ...
                  <CardContent className="text-center space-y-2">
                    <Avatar className={`h-20 w-20 mx-auto ${isCurrentUser(topUsers[0]) ? "ring-2 ring-primary" : ""}`}>
              ...
              {/* 3rd Place */}
              {topUsers[2] && (
                <Card className={`border-2 border-amber-600 bg-amber-50 dark:bg-amber-900/10 ${isCurrentUser(topUsers[2]) ? "ring-2 ring-primary" : ""}`}>
                  <CardHeader className="text-center pb-3">
                  ...
                  <CardContent className="text-center space-y-2">
                    <Avatar className={`h-16 w-16 mx-auto ${isCurrentUser(topUsers[2]) ? "ring-2 ring-primary" : ""}`}>
              ...
              <div className="space-y-3">
                {listUsers.map((entry: any, index: number) => {
                  ...
                  return (
                    <Card
                      key={entry.userId}
                      className={`overflow-hidden transition-all hover:shadow-md border ${isMe
                    ...
                    >
                      <CardContent className="p-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex items-center gap-3 flex-1">
                            <Avatar className={`h-12 w-12 ${isMe ? "ring-2 ring-primary" : ""}`}>
                              <AvatarImage
                                ...
                                alt={entry.name}
                              />
                              <AvatarFallback className={isMe ? "bg-primary/20 text-primary font-bold" : ""}>
                                {entry.name?.substring(0, 2)}
                              </AvatarFallback>
                            </Avatar>
                            <div className="flex-1">
                              <div className="flex items-center gap-2">
                                <h3 className={`font-semibold ${isMe ? "text-primary" : ""}`}>
                          ...
                          {/* Stats */}
                          <div className="flex flex-col items-end gap-1">
                            <div className={`flex items-center gap-1 text-lg font-bold ${isMe ? "text-primary" : ""}`}>
                              <Trophy className={`h-5 w-5 ${isMe ? "text-primary" : "text-yellow-600"}`} />
