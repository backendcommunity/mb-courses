...
export function StageDetailPage({ landId, stageId, onNavigate }: StageDetailPageProps) {
  ...
  if (!land || !stage) {
    return (
      <div className="flex-1 p-6 flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground mb-4">The stage you're looking for doesn't exist</p>
        <Button onClick={() => onNavigate(routes.landDetail(landId))}>Back to Land</Button>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 space-y-6 p-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" onClick={() => onNavigate(routes.landDetail(landId))}>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-l-4 border-l-[#F2C94C]">
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold mt-1">{stage.totalXP.toLocaleString()}</p>
            <p className="text-xs text-muted-foreground">{stage.completedXP.toLocaleString()} earned</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-[#13AECE]">
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground">
              {stage.challenges.filter((challenge) => challenge.completed).length} completed
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-[#347474]">
          <CardContent className="pt-6">
            ...
            <p className="text-xs text-muted-foreground">To complete all challenges</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-sm text-blue-100">
              ...
              <span>{(stage.totalXP - stage.completedXP).toLocaleString()} XP remaining</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">Complete these challenges to master {stage.title}</p>
      </div>

      <div className="space-y-4">
        {stage.challenges.map((challenge, index) => (
          <Card key={challenge.id} className={!challenge.unlocked ? "opacity-70" : ""}>
            ...
            <CardContent className="space-y-4">
              ...
              {challenge.usedHints > 0 && (
                <div className="flex items-center gap-2 text-sm text-orange-600">
                  ...
                  <span>
                    {challenge.usedHints} hint{challenge.usedHints > 1 ? "s" : ""} used
                  </span>
                </div>
              )}
              ...
              <Button
                ...
                onClick={() => onNavigate(routes.challengeDetail(landId, stageId, challenge.id))}
              >
                <span>{challenge.completed ? "Review" : "Start Challenge"}</span>
        ...
        <Card className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-500/20">
          ...
          <CardContent>
            <p className="text-green-600 mb-4">
              Congratulations! You've completed all challenges in this stage and earned {stage.totalXP.toLocaleString()}{" "}
              XP.
            </p>
